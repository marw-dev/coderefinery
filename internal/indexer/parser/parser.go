...
func GetParser(lang string) Parser {
	// 1. Versuch: High-Quality TreeSitter für Mainstream-Sprachen
	//    (Go, Python, JavaScript, TypeScript)
	if ts := NewTreeSitterParser(lang); ts != nil && ts.SupportsLanguage(lang) {
		return ts
	}

	// 2. Versuch: Go's nativer AST Parser (beste Qualität für Go)
	if lang == "go" {
		return &GoParser{}
	}

	// 3. Fallback: Universal AST Parser
